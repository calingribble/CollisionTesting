...
public class starAnimation{
	...
	static int max_num_of_stars_FRONT = 20*mainFrame.SCALE;
	static int max_num_of_stars_BACK = 50*mainFrame.SCALE;
	...
	static boolean locationsGenerated = false;
	...
	static TimerTask star_movement_FRONT = new TimerTask(){
		public void run() {
			...
			for(int i = 0; i < max_num_of_stars_FRONT; i++){
				if(xLoc_FRONT[i] > mainFrame.WIDTH ){
					xLoc_FRONT[i] = 0;
				}else{
					xLoc_FRONT[i]+=1;
				}
	...
	static TimerTask REANIMATE_STARS = new TimerTask(){
		public void run() {
		}
	};
	public static void generateLocationArrays(){
		for (int i = 0; i < max_num_of_stars_FRONT; i++){
			xLoc_FRONT[i] = rand.nextInt(mainFrame.WIDTH );
		}
		for (int i = 0; i < max_num_of_stars_FRONT; i++){
			yLoc_FRONT[i] = rand.nextInt(mainFrame.HEIGHT );
		}
		for (int i = 0; i < max_num_of_stars_FRONT; i++){
		...
		for (int i = 0; i < max_num_of_stars_BACK; i++){
			xLoc_BACK[i] = rand.nextInt(mainFrame.WIDTH );
		}
		for (int i = 0; i < max_num_of_stars_BACK; i++){
			yLoc_BACK[i] = rand.nextInt(mainFrame.HEIGHT );
		}
		for (int i = 0; i < max_num_of_stars_BACK; i++){
	...
	public static void ANIMATE_STARS(){
		if(!locationsGenerated) generateLocationArrays();
